...
ROOT = Path(".")
OUT_ROOT = ROOT / "data" / "raw" / "aceh_simeulue" / "thermal_depth_nrt"
REPORT_FILE = ROOT / "data" / "physics" / "thermal_depth_download_report.json"
...
def inspect_netcdf(path: Path) -> dict:
    try:
        ...
        ds = xr.open_dataset(path, engine="h5netcdf", cache=False, decode_times=False)
        ...
        depth_values = []
        if depth_name:
            vals = np.asarray(ds[depth_name].values).astype(float).ravel()
        ...
        return {
            ...
            "thetao_dims": list(ds["thetao"].dims) if "thetao" in ds else None,
            "thetao_units": ds["thetao"].attrs.get("units") if "thetao" in ds else None,
        }
...
def main() -> None:
    ...
    for d in dates:
        ...
        out_name = f"thermal_depth_nrt_aceh_{date_str}.nc"
