...
def main() -> None:
    ...
    ap.add_argument("--var3d", default="sal3d", help="folder var untuk netcdf 3D (mis: sal3d)")
    ...
    var3d = str(args.var3d or "sal3d").strip()
    ...
    target_depths = np.arange(
        0.0,
        float(args.max_depth) + 1e-6,
        float(args.step),
        dtype=float,
    )

    # pakai dict untuk mencegah depth duplikat akibat nearest-neighbor
    ...
    for td in target_depths:
        ...
        by_depth[round(d, 3)] = float(val)

    pts = [{"depth_m": z, "sal_psu": by_depth[z]} for z in sorted(by_depth.keys())]

    # ---- output DAILY ----
    out_daily_dir = base_dir / "sal_profile" / "daily"
    out_daily_dir.mkdir(parents=True, exist_ok=True)
    out_daily = out_daily_dir / f"sal_profile_daily_{day}.csv"

    with out_daily.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["depth_m", "sal_psu"])
        w.writeheader()
        for row in pts:
            w.writerow(row)

    # ---- output SERIES (long format) ----
    out_series_dir = base_dir / "sal_profile" / "series"
    out_series_dir.mkdir(parents=True, exist_ok=True)
    out_series = out_series_dir / "sal_profile_daily_profile.csv"
    ...
    with out_series.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["date", "depth_m", "sal_psu"])
        w.writeheader()
            ...
            w.writerow(
                {
                    ...
                    "sal_psu": (row.get("sal_psu") or "").strip(),
                }
            )
    ...
    print(f"[OK] sal_profile DAILY  : {out_daily} (points={len(pts)})")
    print(f"[OK] sal_profile SERIES : {out_series} (date={day}, points={len(pts)})")
