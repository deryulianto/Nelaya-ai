...
def aoi_confidence(region: Optional[dict[str, Any]]) -> str:
    ...
    if rtype in {"island", "island_cluster"}:
        return "medium"

    if rtype == "coastal_sea":
        return "low_medium"
...
def classify_aoi(
    ...
    visible_context_types = {"island", "island_cluster", "coastal_sea"}
    visible = (
        small_allowed
        and legal_zone == "zone_0_4_nm"
        and rtype in visible_context_types
    )
...
def main() -> None:
    ...
    for feat in features:
        ...
        props.update(
            {
                "legal_aoi_version": "0.7.2",
                **aoi,
                "aoi_note": (
                    "Aceh AOI Guardrail v0.7.2 memakai bbox region sementara. "
    ...
    summary = {
        ...
        "message": (
            "Aceh AOI Guardrail v0.7.2 berhasil dibuat. "
