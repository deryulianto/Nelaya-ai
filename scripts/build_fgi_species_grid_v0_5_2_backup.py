...
def main() -> None:
    ...
    def add_relative_ranking(score_key: str, prefix: str) -> None:
        ...
        ranked.sort(
            key=lambda f: to_float((f.get("properties") or {}).get(score_key)) or -1,
            reverse=True,
        )
        ...
        for rank, feat in enumerate(ranked, start=1):
            ...
            if score is None:
                rel_label = "tidak_tersedia"
            elif score < 0.45:
                rel_label = "rendah"
            elif rank <= 10:
                rel_label = "prioritas_observasi_harian"
            elif percentile >= 0.75:
                rel_label = "menarik_dipantau"
            elif percentile >= 0.50:
                rel_label = "sedang_dipantau"
            else:
                rel_label = "indikatif_rendah"

            props[f"{prefix}_relative_label"] = rel_label
