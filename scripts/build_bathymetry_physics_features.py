...
"""
NELAYA-AI Bathymetry Physics Features v0.2 SAFE

Perubahan dari v0.1:
- Lebih aman terhadap segmentation fault.
- Membaca GEBCO dengan engine h5netcdf secara eksplisit.
- Coarsen / turunkan resolusi dulu, baru hitung slope.
- Tidak menghitung gradient pada grid GEBCO high-resolution penuh.
- Output tetap:
...
# Kurangi risiko benturan thread native library di WSL/Linux
...
def normalize_lon_if_needed(ds: xr.Dataset) -> xr.Dataset:
    lon = ds["lon"]
...
def open_dataset_safe(path: Path, engine: str) -> xr.Dataset:
    """
    Gunakan h5netcdf default agar menghindari sebagian crash netCDF4/HDF5.
    Jika h5netcdf belum ada:
...
def compute_slope_from_coarse_depth(depth: xr.DataArray) -> xr.DataArray:
    """
    Hitung slope dari grid yang sudah di-coarsen.
    Ini lebih aman daripada gradient pada grid GEBCO resolusi tinggi.
    """
    ...
    if lat_res is None or lon_res is None:
        raise ValueError("Cannot estimate coarse grid resolution.")
    ...
    return xr.DataArray(
        ...
        attrs={
            "long_name": "Approximate bathymetric slope from coarsened depth",
            "units": "m m-1",
            "note": "Computed after coarsening GEBCO to reduce memory/native-library risk.",
        },
    )
...
def classify_depth_zone(depth: xr.DataArray) -> xr.DataArray:
    ...
    zone.attrs = {
        ...
        "codes": (
            "0=land_or_unknown, 1=coastal_0_50m, "
            "2=shelf_50_200m, 3=upper_slope_200_1000m, 4=deep_gt_1000m"
        ),
    }
    return zone
...
def build_species_support_scores(
    ...
    """
    Skor ini bukan prediksi ikan.
    Ini hanya layer dukungan struktur kedalaman untuk digabung nanti
    dengan SST, CHL, arus, wave, wind, SSH, dan validasi lapangan.
    """
    ...
    small_pelagic.attrs = {
        ...
        "units": "0-1",
        "note": "Static bathymetry support; not direct fish abundance.",
    }
    ...
    medium_pelagic.attrs = {
        ...
        "units": "0-1",
        "note": "Static bathymetry support; must be fused with dynamic ocean variables.",
    }
    ...
    demersal.attrs = {
        ...
        "units": "0-1",
        "note": "Static bathymetry support; not direct catch prediction.",
    }
...
def main() -> None:
    ...
    parser.add_argument("--out", default="data/physics")
    parser.add_argument("--coarsen-factor", type=int, default=20)
    parser.add_argument("--engine", default="scipy", choices=["scipy", "netcdf4", "h5netcdf", "auto"])
    parser.add_argument("--geojson-threshold", type=float, default=0.65)
    ...
    print("=" * 78, flush=True)
    print("NELAYA-AI Bathymetry Physics Features v0.2 SAFE", flush=True)
    print("=" * 78, flush=True)
    ...
    print("Opening dataset safely...", flush=True)
    ds_raw = open_dataset_safe(input_file, engine=args.engine)

    print("Subsetting Aceh bbox...", flush=True)
    ds_sub = subset_aceh(ds_raw)

    if "elevation" not in ds_sub:
        raise SystemExit("Variable 'elevation' not found in input file.")

    factor = int(args.coarsen_factor)

    print("Building ocean depth from GEBCO elevation...", flush=True)
    elevation = ds_sub["elevation"]

    # GEBCO: laut negatif, darat positif.
    depth_hr = xr.where(elevation < 0, -elevation, np.nan)
    ocean_mask_hr = xr.where(elevation < 0, 1.0, 0.0)

    depth_hr.name = "depth_m_hr"
    ocean_mask_hr.name = "ocean_mask_hr"

    print("Coarsening depth and ocean fraction first...", flush=True)
    depth_c = depth_hr.coarsen(lat=factor, lon=factor, boundary="trim").mean(skipna=True).load()
    ocean_fraction = ocean_mask_hr.coarsen(lat=factor, lon=factor, boundary="trim").mean(skipna=True).load()

    depth_c.name = "depth_m"
    depth_c.attrs = {
    ...
    print("Masking weak ocean-fraction cells...", flush=True)
    valid_ocean_cell = ocean_fraction >= 0.25
    depth_c = depth_c.where(valid_ocean_cell)

    print("Computing slope from coarsened grid...", flush=True)
    slope_c = compute_slope_from_coarse_depth(depth_c)

    print("Computing depth zones and topographic scores...", flush=True)
    ...
    shelf_break_score.attrs = {
        ...
        "units": "0-1",
        "note": "Static topographic transition layer for Physics-informed FGI.",
    }
    ...
    ds_out.attrs = {
        ...
        "note": (
            "Static bathymetry-derived physics layer for FGI. "
            "This is not a Navier-Stokes solver; it extracts topographic structure "
            "from GEBCO for Physics-informed FGI."
        ),
    }
    ...
    print("Saving NetCDF output...", flush=True)
    save_engine = args.engine if args.engine in ["scipy", "netcdf4", "h5netcdf"] else None
    ds_out.to_netcdf(nc_out, engine=save_engine)
    ...
    summary = {
        ...
        "interpretation": {
            "what_this_means": (
                "Bathymetry layer is ready as a static topographic intelligence layer. "
                "It can be fused with SST, CHL, current, SSH, wave, wind, and field validation "
                "to build Physics-informed FGI."
            ),
            "caution": (
                "These scores are not fish abundance predictions. "
                "They are structural habitat-support features derived from bathymetry only."
            ),
            "recommended_next_step": (
                "Fuse this static bathymetry layer with dynamic ocean physics: current, front, "
                "convergence, vorticity, SST gradient, CHL gradient, SSH gradient, wave, and wind."
            ),
        },
    }
    ...
    print(f"  cat {json_out} | jq '.grid'", flush=True)
    print(f"  cat {json_out} | jq '.stats.shelf_break_score'", flush=True)
    print(f"  cat {json_out} | jq '.top_cells.shelf_break_score[0:10]'", flush=True)
    print("=" * 78, flush=True)
